...
public class RegistrationPatientAdmin extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        loadingDialog = new LoadingDialog(RegistrationPatientAdmin.this);
        ...
        if (name.equals("") || roll_no.equals("") || class_std.equals("") || password.equals("")) {
            ...
            referenceData.child("patient").child(roll_no).addListenerForSingleValueEvent(new ValueEventListener() {
                ...
                public void onDataChange(@NonNull DataSnapshot snapshot) {
                    if (snapshot.exists()){
                        loadingDialog.dismissDialog();
                        Toast.makeText(RegistrationPatientAdmin.this, "This roll no is already registered.", Toast.LENGTH_SHORT).show();
                    }
                    else {
                        referenceData.child("patient").child(roll_no).setValue(patientUpload).addOnCompleteListener(new OnCompleteListener<Void>() {
                            ...
                            public void onComplete(@NonNull Task<Void> task) {
                                loadingDialog.dismissDialog();
                                Toast.makeText(RegistrationPatientAdmin.this, "Success", Toast.LENGTH_SHORT).show();
                                startActivity(new Intent(RegistrationPatientAdmin.this, AdminMainActivity.class));
                                finish();
                            }
                        });
    ...
    public void onBackPressed() {
        Intent intent2 = new Intent(RegistrationPatientAdmin.this, AdminMainActivity.class);
